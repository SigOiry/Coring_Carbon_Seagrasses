...
<div class="warning-overlay-box" id="warningOverlay">
  ...
  <h1>⚠️ Warning</h1>
  <p>The intertidal make of the site still needs work because a lot of saltmarsh are included.</p>
</div>
...
leaflet() %>%
  ...
  addPolygons(
    ...
    fillColor   = "white",    # fill color (same as border)
    fillOpacity = 0.3,
    popup       = ~paste0(
      "<strong>Area:</strong> ",
      round(area_km2, 2), " km²"
...
It_mask <- "Data/Aveiro_Lagoon/SHP/mask_land_intertidal_Aveiro.shp" %>% 
  vect() %>% 
  dplyr::filter(Type == "Intertidal")
...
anim_save("Plots/Animation_SPC_RA.gif", animation = rendered_SPC)
